...
- 監視対象は最大 50 アプリ
- **日本と米国の両方**の Google Play を追跡（評価は国ごとに集計されており、実測で 0.6 の差が出ることもある）
...
Google Play は評価・レビュー・コンテンツレーティング・通貨・説明文を国別に返します。
YouTube の実測では日本 3.24 / 米国 3.84 と評価に 0.6 の差がありました。
一方、評価件数とインストール数は全世界共通の値なので国間で比較できません。

対象国は `src/config.ts` の `COUNTRIES` で定義しています。
国を増やすと 1 アプリあたりのサブリクエストが 2 ずつ増えるため、
無料枠（50/実行）との兼ね合いで 3 か国程度が上限です。
