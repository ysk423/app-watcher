...
| データ | 増え方 | 既定設定での概算 |
|---|---|---|
| レビュー | 100 アプリ × 50 件 × 保持日数 | 90 日で最大 300 MB |
| スナップショット | 100 アプリ × 日数（削除しない） | 1 年で約 40 MB |
| 実行履歴 | 1 日約 200 行 | 30 日で約 1 MB |
| AI 分析 | アプリごと 30 件 | 数 MB |

**容量を左右するのはレビューだけ**である。1 行あたり 0.5〜1 KB の本文を持つうえ、
アプリ数と保持日数の積で増える。レビュー保持日数を 180 日にすると最悪ケースで上限を超える。

一方、実行履歴は 365 日に伸ばしても 15 MB 程度で影響しない。

上の数字はすべて「100 アプリ全てが毎日 50 件の新規レビューを持つ」最悪ケースである。
レビュー ID で重複排除しているため、投稿の少ないアプリでは実際の増加はずっと小さい。
