...
const MAX_REVIEWS_FOR_APP_ANALYSIS = 15;
...
export async function buildAppAnalysisPrompt(
  ...
  const [snapshots, lastTwo, recent7, recent30, reviews] = await Promise.all([
    getSnapshots(db, packageName, ANALYSIS_HISTORY_DAYS + 1),
    getLastTwoSnapshots(db, packageName),
    reviewSummary(db, packageName, isoDaysAgo(7)),
    reviewSummary(db, packageName, isoDaysAgo(30)),
    listRecentReviewsForAi(db, packageName, isoDaysAgo(7), MAX_REVIEWS_FOR_APP_ANALYSIS),
  ]);

  if (!lastTwo.current) return null; // データがまだ 1 件も無いなら分析しない
  ...
  lines.push(`- 評価件数: ${formatCount(lastTwo.current.ratings)}`);
  lines.push(`- インストール数: ${lastTwo.current.installs ?? '不明'}`);
  if (lastTwo.current.recent_changes) {
    lines.push(`- What's New: ${truncate(lastTwo.current.recent_changes, 500)}`);
  }
  ...
  if (reviews.length > 0) {
    lines.push(`\n# 直近のレビュー(最大 ${MAX_REVIEWS_FOR_APP_ANALYSIS} 件)`);
  ...
  lines.push(
    `\n# 依頼\n上記データだけを根拠に、次の観点で 400 字程度にまとめてください。\n` +
...
export async function buildGlobalAnalysisPrompt(db: D1Database): Promise<string | null> {
  ...
  for (const app of apps) {
    const pair = grouped.get(app.package_name);
    ...
    const versionChanged = oldest && oldest.version !== latest.version ? `${oldest.version} → ${latest.version}` : 'なし';
    lines.push(
  ...
  lines.push(
    `\n# 依頼\n上記データだけを根拠に、次を 400 字程度でまとめてください。\n` +
...
export async function buildQaPrompt(db: D1Database, question: string): Promise<string> {
  ...
  for (const app of apps) {
    ...
    lines.push(
      `- ${app.title ?? app.package_name}(${app.package_name}) 状態:${app.status}` +
        ` 評価:${formatScore(latest?.score ?? null)}(${delta(oldest?.score ?? null, latest?.score ?? null)})` +
        ` 評価件数:${formatCount(latest?.ratings ?? null)}(${delta(oldest?.ratings ?? null, latest?.ratings ?? null)})` +
  ...
  const mentioned = apps
    .filter((a) => {
      const title = a.title ?? '';
      return (
        question.includes(a.package_name) ||
        (title.length >= 2 && question.includes(title))
      );
    })
    .slice(0, 2);

  for (const app of mentioned) {
    const reviews = await listRecentReviewsForAi(db, app.package_name, isoDaysAgo(30), MAX_REVIEWS_FOR_QA);
    if (reviews.length === 0) continue;
    lines.push(`\n# ${app.title ?? app.package_name} の直近レビュー(最大 ${MAX_REVIEWS_FOR_QA} 件)`);
    for (const r of reviews) {
      lines.push(`- [★${r.score ?? '-'} ${r.review_date.slice(0, 10)}] ${truncate(r.text ?? '', 300)}`);
    }
  }
...
function groupEndpoints(
  ...
  for (const row of rows) {
    const entry = map.get(row.package_name) ?? { latest: null, oldest: null };
    if (row.is_latest === 1) entry.latest = row;
    else entry.oldest = row;
    map.set(row.package_name, entry);
  }
  return map;
}
